  ...
  <section v-if="metrics" class="section">
    <div class="metrics-grid">
      <article class="metric-card">
        ...
        <div class="metric-value metric-positive">{{ metrics.savedHours }}h</div>
      ...
      <article class="metric-card">
        ...
        <div class="metric-value metric-positive">¥{{ metrics.estimatedSavingCny }}</div>
      ...
      <article class="metric-card">
        ...
        <div class="metric-note">{{ metrics.diagnosedTickets }}/{{ metrics.totalTickets }} 已完成初诊</div>
      </article>
      ...
      <article class="metric-card">
        ...
        <div class="metric-value metric-warning">{{ percent(metrics.humanConfirmationRate) }}</div>
      ...
      <article class="metric-card">
        ...
        <div class="metric-value metric-danger">{{ metrics.riskTicketCount }}</div>
      ...
      <div class="ticket-list">
        ...
        >
          ...
          <div class="ticket-meta">
            <span class="pill">{{ ticket.customer.name }} · {{ ticket.customer.city }}</span>
            <span class="pill green">可节省 {{ ticket.estimatedManualMinutes - ticket.aiAssistedMinutes }} 分钟</span>
...
import { BadgeDollarSign, Clock3, GitBranch, ShieldAlert, UserCheck } from 'lucide-vue-next'
import type { RoiMetric, TicketPriority, TicketStatus, TicketWithRelations } from '../types/serviceops.ts'
...
const importantTickets = computed(() => {
  ...
  return [...props.tickets]
    .sort((a, b) => {
      const riskA = priorityRank[a.priority] * 100 + (a.estimatedManualMinutes - a.aiAssistedMinutes)
      const riskB = priorityRank[b.priority] * 100 + (b.estimatedManualMinutes - b.aiAssistedMinutes)
      return riskB - riskA
    })
    .slice(0, 4)
})
